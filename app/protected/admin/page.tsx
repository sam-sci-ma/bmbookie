...
export default async function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: "Active Bookings", mobileLabel: "Active", val: (activeRes.count || 0).toString(), icon: CalendarCheck2, color: "text-primary", bg: "bg-primary/10" },
  ...
  return (
    <div className="space-y-6 md:space-y-10 pb-10 animate-in fade-in duration-700 px-4 md:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="group p-5 md:p-8 bg-card border border-border rounded-[1.5rem] md:rounded-[2rem] transition-all duration-300 hover:border-primary/50 shadow-sm">
        ...
        <div className="grid gap-3 md:gap-4">
          {todayEvents.length > 0 ? (
            todayEvents.map((event) => (
              <div key={event.id} className="bg-card border border-border p-4 md:p-6 rounded-2xl md:rounded-[1.5rem] flex flex-col md:flex-row md:items-center justify-between group hover:border-primary/30 transition-all gap-4">
                <div className="flex items-center gap-4 md:gap-6">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-xs md:text-sm font-bold text-foreground uppercase group-hover:text-primary transition-colors truncate">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-1">
                      <p className="text-[9px] md:text-[10px] text-muted-foreground flex items-center gap-1 font-bold uppercase tracking-wider">
                        <MapPin size={10} className="text-primary" /> {event.rooms?.name}
                      </p>
                      <p className="text-[9px] md:text-[10px] text-muted-foreground flex items-center gap-1 font-bold uppercase tracking-wider">
